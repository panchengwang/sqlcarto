...
sym_shape_path_t* sym_shape_path_parse_from_json(json_object* obj) {
    uint8_t closed = 0;
    JSON_GET_BOOLEAN(obj, "closed", closed);

    json_object* objstroke;
    JSON_GET_OBJECT(obj, "stroke", objstroke);
    sym_stroke_t* stroke = sym_stroke_parse_from_json(objstroke);
    if (!_sym_parse_ok) return NULL;

    json_object* objfill;
    JSON_GET_OBJECT(obj, "fill", objfill);
    sym_fill_t* fill = sym_fill_parse_from_json(objfill);
    if (!_sym_parse_ok) return NULL;

    double offset_along_line = 0.0;
    JSON_GET_DOUBLE(obj, "offsetalongline", offset_along_line);
    json_object* arrsubpaths;
    JSON_GET_ARRAY(obj, "subpaths", arrsubpaths);

    int32_t nsubpaths = json_object_array_length(arrsubpaths);
    sym_sub_path_t** subpaths = (sym_sub_path_t**)malloc(sizeof(sym_sub_path_t*) * nsubpaths);
    ...
    path->closed = closed;
    path->nsubpaths = nsubpaths;
    path->subpaths = subpaths;
    path->stroke = stroke;
    path->fill = fill;
    path->offsset_along_line = offset_along_line;
...
json_object* sym_shape_path_to_json(sym_shape_path_t* path) {
    ...
    JSON_SET_DOUBLE(obj, "offsetalongline", path->offsset_along_line);
